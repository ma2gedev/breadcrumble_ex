defmodule BreadcrumbleTest do
  ...
  test "it adds a breadcrumb" do
    ...
    assert Map.get(conn.assigns, :breadcrumb_trails) == [
  ...
  test "it adds a breadcrumb trails" do
    ...
    assert Map.get(conn.assigns, :breadcrumb_trails) == [
             [[name: "name10", url: "/url10"]],
             [[name: "name20", url: "/url20"]]
           ]

    conn = add_breadcrumb(conn, name: "name11", url: "/url11")

    assert Map.get(conn.assigns, :breadcrumb_trails) == [
             [[name: "name10", url: "/url10"], [name: "name11", url: "/url11"]],
             [[name: "name20", url: "/url20"]]
           ]
    ...
    assert Map.get(conn.assigns, :breadcrumb_trails) == [
             [[name: "name10", url: "/url10"], [name: "name11", url: "/url11"]],
             [[name: "name20", url: "/url20"], [name: "name21", url: "/url21"]]
           ]
  end
end
